    ...
    public function getInvoiceList()
    {
        ...
        return Datatables::of($orderlist)
                        ...
                        ->editColumn("id", '<a href="{{url("/")}}/invoice/edit/{{ $id }}" class="btn btn-primary" id="btnEdit">'
                                ...
                                . '<a href="{{url("/")}}/invoice/print/{{ $id }}" class="btn btn-warning "  style="margin-left: 10px"><span class="fa fa-print"></span></a>')
    ...
    public function printInvoice($id)
    {
        ...
        if (!empty($id)) {
            ...
            $invoiceOrder = DB::table('invoice')
                    ...
                    ->where('invoice.id', $id)
                    ->first();
            $customer = DB::table('customers')->where('id', $invoiceOrder->customer_id)->first();
            //Get list of Parts order for Invoice
            $invoiceOrderList = DB::table('invoice_order_list')
                    ->select(array('invoice_order_list.*', 'part_number.*', 'invoice_order_list.id as invoiceOrderId'))
                    ->leftJoin('order_list', 'order_list.id', '=', 'invoice_order_list.order_id')
            ...
            $PO = DB::table('purchase_order')->get();
            $pdf->loadView('invoice.print', array('cust' => $customer, 'invoiceOrder' => $invoiceOrder, 'invoiceSKUOrder' => $invoiceOrderList, 'po' => $PO));
